...
/**
 * A spectra object contains a collection of PowerSpectrum objects. It will
 * extract the peaks only when the method is called.
 *
 */
public class Spectrogram {
    ...
    /**
     * The collection of PowerSpectrum objects, which each represent a
    ...
    /**
     * The size of the power spectrum array is half the number of samples per
     * interval.
     */
    ...
    /**
     * Extracts the peaks inside the spectrogram from each frequency bin by
     * filtering the values that are less than the corresponding values in
     * neighboring spectra.
     */
    public void extractPeaks() {
        ...
        /*
         * for(Peak peak :globalPeaks){ System.out.println(peak.getTime() + " "
         * + peak.getFrequency() + " ");
		}
         */
    }
     ...
     * @return Whether the local peak is still considered a global peak. True if
     * it still is, false otherwise.
     */
    private boolean checkAdjacentSpectra(int index, int frequency, double power) {
        ...
        for (int i = (-3 + index); i <= (3 + index); i++) {
            if (i >= 0 && i < numberOfSpectra && i != index) {
                ...
                if ((power - oPower) < PowerSpectrum.PEAK_THRESHOLD) {
                    return false;
                }
                if ((power - getAveragePowerAt(i)) < PowerSpectrum.PEAK_THRESHOLD) {
                    return false;
                }
            }
        }
    ...
    /**
     * Gets all the peaks from the spectrogram. The peaks are already in order,
     * first by time of occurrence, then by the frequency values. No sort was
     * implemented. The peaks were added simply from one end of the spectrogram
