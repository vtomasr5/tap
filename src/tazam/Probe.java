...
/**
 * This class represents a probe that the program will collect for each audio
 * file.
 *
 */
public class Probe implements Comparable<Probe>, Serializable {
    ...
    /**
     * Constructs a probe from two peaks. The first is the anchor peak, which is
     * the base value from which the first frequency will be constructed.
     *
    ...
    public boolean equals(Object otherObject) {
        ...
        if (this.dt == otherProbe.dt && this.firstFrequency == otherProbe.firstFrequency
                && this.secondFrequency == otherProbe.secondFrequency) {
            return true;
        }
        return false;
    }
    ...
    public int compareTo(Probe o) {
        ...
        if (this.firstFrequency > o.firstFrequency) {
            return 1;
        }
        if (this.firstFrequency < o.firstFrequency) {
            return -1;
        }
        if (this.secondFrequency > o.secondFrequency) {
            return 1;
        }
        if (this.secondFrequency < o.secondFrequency) {
            return -1;
    ...
    /**
     * Computes the hash value of the probe. When we serialize the map, we no
