...
public class SpectrogramProbesExtractor {
    ...
    /**
     * Constructs a probe(if possible) by using the parameters specified by the
     * user. The target region is a rectangle bounded by the TIME_OFFSET along
     * the time axis and the FREQ_OFFSET along the frequency bins.
     *
    ...
    private void createProbe(Peak anchorPeak, int baseIndex) {
        ...
        for (int i = baseIndex; i < p.size(); i++) {
            ...
            if (otherTime > targetTimeBound) {
                return;//we have jumped out of the target zone.
            }
